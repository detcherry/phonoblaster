...
class Api(object):
  ...
  Example usage:
    ...
    There are many other methods, including:
	  ...
	  >>> api.GetBigProfileImage(screen_name)
  ...
  def GetBigProfileImage(self, screen_name=None):
	if not screen_name and not self._oauth_consumer:
		raise TwitterError("User must be specified if API is not authenticated")
	url = '%s/users/profile_image' % self.base_url
	parameters = {}
	parameters["screen_name"] = screen_name
	parameters["size"] = "reasonably_small"
	response = self._FetchUrl(url, parameters=parameters)
	return response
